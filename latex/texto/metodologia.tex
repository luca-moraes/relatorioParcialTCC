...
Além da fonte principal de dados, planejamos também serão utilizar dados das respostas dissertativas dos alunos de Ciência da Computação do Centro Universitário FEI para questões dissertativas do ENADE.
...
Assumindo que dois textos possuem exatamento o mesmo conteúdo, sua similaridade poderia ser descrita em uma escala percentual como 100\%.
...
A proposta da métrica pode ser matemáticamente descrita como na equação da Figura \ref{figure:2} 
\begin{figure}[h!]
\[Métrica1 = \frac{fator1*peso_{1} + fator2*peso_{2} + fator3*peso_{3} + fator4*peso_{4}}{\sum_{i=1}^{4}peso_{i}}\]
\caption{Equação da primeira métrica proposta.}\label{figure:2}
\end{figure}

Onde o \textit{fator1} são as propriedades sintáticas do texto, o \textit{fator2} representa o SFD, o \textit{fator3} é a distância de cossenos e o \textit{fator4} é referente as \textit{keywords}. 

Como resultado a métrica deverá fornecer um númeor de zero a cem que, quanto maior, indica maior proximidade semântica entre os textos e uma maior probabilidade de acerto da resposta fornecida por um aluno. 

No caso da segunda métrica, o diferencial seria adição dos dois outros fatores para mensurar o grau de parafraseamento e a quantidade de modificações, sendo respectivamente o \textit{fator5} e o \textit{fator6} como descrito na equação da Figura \ref{figure:3}

\begin{figure}[h!]
\tiny \[Métrica2 = \frac{fator1*peso_{1} + fator2*peso_{2} + fator3*peso_{3} + fator4*peso_{4} + fator5*peso_{5} + fator6*peso_{6}}{\sum_{i=1}^{6}peso_{i}}\]
\caption{Equação da segunda métrica proposta.}\label{figure:3}
\end{figure}
